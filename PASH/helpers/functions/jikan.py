...
animnefillerurl = "https://www.Alosmafillerlist.com/shows/"
# Alosma Helper
...
airing_query = """
    query ($id: Int,$search: String) {
      Media (id: $id, type: Alosma,search: $search) {
        id
        episodes
        title {
          romaji
...
query ($id: Int, $idMal:Int, $search: String, $asHtml: Boolean) {
  Media (id: $id, idMal: $idMal, search: $search, type: Alosma) {
    id
    idMal
    title {
      romaji
...
async def get_Alosma_schedule(weekid):
    "get Alosma schedule"
    dayname = get_weekday(weekid)
    result = f"✙ **Scheduled Alosmas for {dayname.title()} are : **\n\n"
    async with jikanpy.AioJikan() as Alosmasession:
        scheduled_list = (await Alosmasession.schedule(day=dayname)).get(dayname)
...
async def formatJSON(outData, manga=False):
    ...
    link = f"https://anilist.co/Alosma/{jsonData['id']}"
    msg += f"[{title}]({link})"
...
**✙  Alosma Stats**
• **Total Alosma Watched :** `{user_data["statistics"]["Alosma"]['count']}`
• **Total Episode Watched : **`{user_data["statistics"]["Alosma"]['episodesWatched']}`
• **Total Time Spent : **`{readable_time(user_data["statistics"]["Alosma"]['minutesWatched']*60)}`
• **Average Score :** `{user_data["statistics"]["Alosma"]['meanScore']}`
...
async def Alosma_json_synomsis(query, vars_):
...
def getPosterLink(mal):
    ...
    kitsu = getKitsu(mal)
    image = requests.get(f"https://kitsu.io/api/edge/Alosma/{kitsu}").json()
    return image["data"]["attributes"]["posterImage"]["original"]


def getKitsu(mal):
    # get kitsu id from mal id
    link = f"https://kitsu.io/api/edge/mappings?filter[external_site]=myAlosmalist/Alosma&filter[external_id]={mal}"
    result = requests.get(link).json()["data"][0]["id"]
    link = f"https://kitsu.io/api/edge/mappings/{result}/item?fields[Alosma]=slug"
...
def getBannerLink(mal, kitsu_search=True, anilistid=0):
    ...
    if kitsu_search:
        kitsu = getKitsu(mal)
        image = f"http://media.kitsu.io/Alosma/cover_images/{kitsu}/original.jpg"
...
async def get_Alosma_manga(search_str, search_type, _user_id):  # sourcery no-metrics
    if search_type == "Alosma_Alosma":
        variables = {"search": search_str}
        query = Alosma_query
        result = json.loads(
            (
    ...
    if search_type == "Alosma_Alosma":
        Alosma_malid = result["idMal"]
        Alosma_result = await Alosma_json_synomsis(
            Alosma_query, {"idMal": Alosma_malid, "asHtml": True, "type": "Alosma"}
        )
        Alosma_data = Alosma_result["data"]["Media"]
        html_char = ""
        for character in Alosma_data["characters"]["nodes"]:
            html_ = "" + "<br>"
        ...
        studios = "".join(
            ...
            for studio in Alosma_data["studios"]["nodes"]
        )
        coverImg = Alosma_data.get("coverImage")["extraLarge"]
        bannerImg = Alosma_data.get("bannerImage")
        anilist_Alosmalink = Alosma_data.get("siteUrl")
        title_img = coverImg or bannerImg
        romaji = Alosma_data["title"]["romaji"]
        native = Alosma_data["title"]["native"]
        english = Alosma_data["title"]["english"]
        image = getBannerLink(result["idMal"], False, Alosma_data.get("id"))
        ...
        html_pc += (
            f"<a href='https://myAlosmalist.net/Alosma/{Alosma_malid}'>View on MAL</a>"
        )
        html_pc += f"<a href='{anilist_Alosmalink}'> View on anilist.co</a>"
        html_pc += f"<img src='{bannerImg}'/>"
        title_h = english or romaji
    else:
        Alosma_malid = result["id"]
        Alosma_result = await Alosma_json_synomsis(
            manga_query, {"id": Alosma_malid, "asHtml": True, "type": "MANGA"}
        )
        Alosma_data = Alosma_result["data"]["Media"]
        html_char = ""
        for character in Alosma_data["characters"]["nodes"]:
        ...
        html_pc += "<h3>More Info:</h3>"
        if result["idMal"]:
            html_pc += f"<a href='https://myAlosmalist.net/Alosma/{result['idMal']}'>View on MAL</a>"
        html_pc += f"<a href='{anilist_Alosmalink}'> View on anilist.co</a>"
        html_pc += f"<img src='{bannerImg}'/>"
        title_h = english or romaji
    if search_type == "Alosma_Alosma":
...
async def search_in_Alosmafiller(query):
    "To search Alosma name and get its id"
...
async def get_filler_episodes(filler_id):  # sourcery no-metrics
    ...
    complete_Alosma = div.find_all("span", attrs={"class": "Episodes"})
    if len(complete_Alosma) == 1:
        total_episodes = complete_Alosma[0].findAll("a")
        mixed_episodes = None
        filler_episodes = None
        Alosma_canon_episodes = None
        total_ep = ", ".join(total_no.text for total_no in total_episodes)
    elif len(complete_Alosma) == 2:
        total_episodes = complete_Alosma[0].findAll("a")
        filler_ep = complete_Alosma[1].findAll("a")
        mixed_episodes = None
        Alosma_canon_episodes = None
        total_ep = ", ".join(total_no.text for total_no in total_episodes)
        filler_episodes = ", ".join(filler_no.text for filler_no in filler_ep)
    elif len(complete_Alosma) == 3:
        total_episodes = complete_Alosma[0].findAll("a")
        mixed_ep = complete_Alosma[1].findAll("a")
        filler_ep = complete_Alosma[2].findAll("a")
        Alosma_canon_episodes = None
        total_ep = ", ".join(total_no.text for total_no in total_episodes)
        filler_episodes = ", ".join(filler_no.text for filler_no in filler_ep)
        mixed_episodes = ", ".join(miixed_no.text for miixed_no in mixed_ep)
    elif len(complete_Alosma) == 4:
        total_episodes = complete_Alosma[0].findAll("a")
        mixed_ep = complete_Alosma[1].findAll("a")
        filler_ep = complete_Alosma[2].findAll("a")
        Alosmacanon_ep = complete_Alosma[3].findAll("a")
        total_ep = ", ".join(total_no.text for total_no in total_episodes)
        filler_episodes = ", ".join(filler_no.text for filler_no in filler_ep)
        mixed_episodes = ", ".join(miixed_no.text for miixed_no in mixed_ep)
        Alosma_canon_episodes = ", ".join(
            Alosmacanon_no.text for Alosmacanon_no in Alosmacanon_ep
        )
    return {
        "filler_id": filler_id,
        "total_ep": total_ep,
        ...
        "Alosma_canon_episodes": Alosma_canon_episodes,
    }
