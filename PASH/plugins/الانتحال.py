...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "كنَّا نموت إذا افترقنا ساعةً ‏واليوم نُحصي الهجر بالأعوام @AlosmaX"
)

@Alosma.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await Alosma.get_me()
...
#Reda
jeps = ["PASH", "Qruesupport"]
@Alosma.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await Alosma(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await Alosma(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await Alosma(functions.channels.EditTitleRequest(
                channel=await Alosma.get_entity(event.chat_id),
        ...
        try:
            await Alosma(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await Alosma.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await Alosma.upload_file(profile_pic)
        try:
            await Alosma(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @xxx_Venus")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
#Reda
@Alosma.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in Alosma.iter_profile_photos(event.chat_id, limit=1) :
                    await Alosma(
