...
IFFUCI_ACTIVE_BRANCH_NAME = "PASH"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/PASH"
...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**᯽︙ قام مطورين السورس بتحديث الفينوس**\n᯽︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس الفينوس بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [‏z1zzzzzzz1.chat_id, ‏z1zzzzzzz1.id])
    except Exception as e:
        LOGS.error(e)
    ups_rem.fetch(ac_br)
    repo.git.reset("--hard", "FETCH_HEAD")
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/PASH", force=True)
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس الفينوس انـتـظـر**")
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("PASH", origin.refs.PASH)
        repo.heads.PASH.set_tracking_branch(origin.refs.PASH)
        repo.heads.PASH.checkout(True)
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس الفينوس ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس الفينوس انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ جارِ تحديث ريبو التنصيب لسورس الفينوس **")
    off_repo = "https://github.com/almul8ab/VenusIQ"
    os.chdir("/app")
    try:
        txt = (
        ...
        origin = repo.create_remote("upstream", off_repo)
        origin.fetch()
        repo.create_head("PASH", origin.refs.master)
        repo.heads.PASH.set_tracking_branch(origin.refs.master)
        repo.heads.PASH.checkout(True)
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("**᯽︙ جارِ اعادة تنصيب سورس الفينوس, انتظر قليلاً ..**")
    await deploy(event, repo, ups_rem, ac_br, txt)


progs = [6330221767,6299015318,‏6801329147,5835316914,6759899119]
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("** ᯽︙ يتم تحديث سورس الفينوس بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
