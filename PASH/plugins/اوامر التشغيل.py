...
async def _(event):
    "Restarts the bot !!"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**⌔︙الفينوس ↻** \n" "**᯽︙ تم اعادة تشغيل السورس بنجاح ✅ ↻**")
    ‏z1zzzzzzz1 = await edit_or_reply(event, "᯽︙ سيتم اعادة التشغيل انتظر ")
    await event.edit("0%\n▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒")
    ...
    if event.reply_to and event.sender_id in JOKRDEV:
        ...
        if owner_id == Alosma.uid:
            if event.message.message == "اعادة تشغيل":
                ‏z1zzzzzzz1 = await event.reply("**᯽︙ سيتم اعادة التشغيل بأمر من المطور **")
                try:
                    ulist = get_collectionlist_items()
                    for i in ulist:
                        if i == "restart_update":
