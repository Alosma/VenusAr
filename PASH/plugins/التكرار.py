...
async def spam_function(event, PASH, Alosma, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(Alosma[0])
    if len(Alosma) == 2:
        spam_message = str(Alosma[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await PASH.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and PASH.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            PASH = await event.client.send_file(
                event.chat_id, PASH, caption=PASH.text
            )
            await _catutils.unsavegif(event, PASH)
        ...
        if BOTLOG:
            ...
            PASH = await event.client.send_file(BOTLOG_CHATID, PASH)
            await _catutils.unsavegif(event, PASH)
        return
    elif event.reply_to_msg_id and PASH.text:
        spam_message = PASH.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@Alosma.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    PASH = await event.get_reply_message()
    Alosma = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(Alosma[0])
    except Exception:
    ...
    await spam_function(event, PASH, Alosma, sleeptimem, sleeptimet)

@Alosma.on(admin_cmd(pattern=f"{Mukrr}"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, Alosma, sleeptimem, sleeptimet, DelaySpam=True)


@Alosma.ar_cmd(pattern="تكرار الملصق$")
...
@Alosma.on(events.NewMessage(pattern=r"^.تكرار (\d+) (\d+) (.+)"))
async def share_messages(event):
    ...
    try:
        entity = await Alosma.get_entity(group_link)
    except ValueError:
    ...
    for _ in range(message_count):
        sent_message = await Alosma.send_message(entity, reply_message.text)
        await asyncio.sleep(time_interval)

    await event.edit(f"⌔∮  تم التكرار بنجاح")

@Alosma.ar_cmd(pattern="وسبام (.*)")
async def tmeme(event):
...
@Alosma.on(events.NewMessage(pattern=r"^.نقل (\d+) (\d+) (.+)"))
async def share_messages(event):
    time_interval = int(event.pattern_match.group(1))
    message_count = int(event.pattern_match.group(2))
    group_link = event.pattern_match.group(3)
    ...
    for _ in range(message_count):
        await Alosma.forward_messages(entity, reply_message)
        await asyncio.sleep(time_interval)

    await event.reply(f"⌔∮ تم التكرار بنجاح")

@Alosma.ar_cmd(pattern="ايقاف التكرار ?(.*)")
