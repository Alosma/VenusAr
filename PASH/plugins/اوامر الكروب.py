...
async def ban_user(chat_id, i, rights):
    try:
        await Alosma(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        
@Alosma.on(events.NewMessage(outgoing=True, pattern="ارسل?(.*)"))
...
@Alosma.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await Alosma(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in Alosma.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await Alosma.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@Alosma.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@Alosma.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@Alosma.ar_cmd(pattern="الغاء التفليش")
...
@Alosma.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await Alosma.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
DevJoker = [6205161271,‏6801329147]
...
async def Hussein(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await Alosma.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await Alosma(LeaveChannelRequest(channel_username))
...
@Alosma.ar_cmd(pattern="مغادرة القنوات")
async def Hussein (event):
...
@Alosma.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@Alosma.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
@Alosma.ar_cmd(pattern=r"(?:اضافة|اضافه) اسم (.+)")
...
@Alosma.ar_cmd(pattern=r"(?:منع|حظر) اسم (?!\.list$)(.+)")
...
@Alosma.ar_cmd(pattern=r"القائمة السوداء$")
...
# الكود من كتابة فريق الفينوس بس تسرقة تنشر بقناة الفضايح انتَ وقناتك 🖤
@Alosma.ar_cmd(pattern=r"ذكاء(.*)")
async def hussein(event):
...
is_Reham = False
No_group_Joker = "@xxx_Venus"
# يا يلفاشل هم الك نيه تاخذه وتنشره بسورسك 🤣
active_AlAlosma = []

@Alosma.ar_cmd(pattern=r"الذكاء تفعيل")
async def enable_bot(event):
    global is_Reham
    if not is_Reham:
        is_Reham = True
        active_AlAlosma.append(event.chat_id)
...
@Alosma.ar_cmd(pattern=r"الذكاء تعطيل")
async def disable_bot(event):
    global is_Reham
    if is_Reham:
        is_Reham = False
        active_AlAlosma.remove(event.chat_id)
...
async def reply_to_hussein(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_AlAlosma:
        return
    message = event.message
