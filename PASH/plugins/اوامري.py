...
from Alosma.razan.resources.assistant import *
from Alosma.razan.resources.mybot import *
from PASH import Alosma
from ..core import check_owner
from ..Config import Config

JEP_IC = "https://telegra.ph/file/56a3dd726306259beded6.jpg"
ROE = "**♰ هـذه هي قائمة اوامـر سـورس الفينوس ♰**"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == bot.uid:
            buttons = [
                [Button.inline("🗿 اوامر الادمن 🗿", data="Alosma0")],
                [
...
async def repo(event):
    ...
    response = await bot.inline_query(‏z1zzzzzzz1, "اوامري")
    await response[0].click(event.chat_id)
    await event.delete()


@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"Alosma0")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"jrzst")))
@check_owner
async def _(event):
    butze = [
    [
     Button.inline("التالي", data="tslrzj"),
     Button.inline("رجوع", data="Alosma0")]]
    await event.edit(GRTSTI, buttons=butze)


@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"tslrzj")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"krrznd")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"rozbot")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"Jmrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"r7brz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"sejrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"gro")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"grrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"iiers")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"rfhrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"uscuxrz")))
@check_owner
async def _(event):
    buttons = [[Button.inline("رجوع", data="Alosma0"),]]
