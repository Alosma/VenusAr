...
@Alosma.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        PASH = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        PASH = str(jok).replace("-100", "")
    else:
        try:
            PASH = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await Alosma.send_message(int(chat), event.message)
