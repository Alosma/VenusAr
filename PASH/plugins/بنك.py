...
@Alosma.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@Alosma.ar_cmd(pattern="توب الفلوس(.*)")
...
@Alosma.ar_cmd(pattern="مسح حسابي(.*)")
...
@Alosma.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@Alosma.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@Alosma.ar_cmd(func=lambda m:"راتب")
...
- مثال: .انشاء حساب الرافدين او الفينوس الاسلامي
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
async def thief(message):
    ...
    PASH = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await Alosma.send_file(
                message.chat_id,
                "https://telegra.ph/file/56a3dd726306259beded6.jpg",
                caption=f"سرق [{PASH}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
@Alosma.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "الفينوس الاسلامي":
        bankn = "مصرف الفينوس الاسلامي"
    elif input == "الرافدين":
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "الفينوس الاسلامي":
