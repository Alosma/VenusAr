...
#جميع الحقوق محفوظة لسـورس الفينوس تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    ‏z1zzzzzzz1 = await reply_id(event)
    if sad:
        Alosma = f"**˛ 𝘼𝙇𝙨𝙖𝙝𝙚𝙧 . .**\n"
        Alosma += f"✛━━━━━━━━━━━━━✛\n"
        Alosma += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=Alosma, reply_to=‏z1zzzzzzz1)
