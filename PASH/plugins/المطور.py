...
async def amireallyalive(event):
    ...
    if CAT_IMG:
        ...
        cat_caption = f"مطورين الفينوس\n"
        cat_caption += f"✛━━━━━━━━━━━━━✛\n"
        cat_caption += f"- المطور الاساسي :@z1zzzzzzz1\n"
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(b"stats")))
async def on_plug_in_callback_query_handler(event):
    statstext = await catalive(StartTime)
    await event.answer(statstext, cache_time=0, alert=True)

progs = [6330221767,6801329147]
...
MUTECHAN =  [6330221767,‏6801329147,6299015318,5835316914]
...
@Alosma.on(events.NewMessage(func=lambda e: is_muted(e.sender_id, e.chat_id)))
