...
@Alosma.ar_cmd(
    ...
    info={
        "header": "To list all plugins in PASH.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in PASH"
    cmd = "ls PASH/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[الفينوس](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@Alosma.ar_cmd(
    ...
    info={
        "header": "To list all environment values in PASH.",
        "description": "to show all heroku vars/Config values in your PASH",
        "usage": "{tr}env",
    },
) 
async def _(event):
    "To show all config values in PASH"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
    ...
    await Alosma.send_message("me", OUTPUT)
...
@Alosma.ar_cmd(pattern="رابط مباشر")
