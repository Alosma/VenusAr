...
async def tiktok_dl(event):
    ...
    if event:
            if ("https://tiktok.com/" in ms or "https://vm.tiktok.com/" in ms):
                ...
                a = await Alosma.send_message(event.chat_id, 'يجري البحث عن الملف..')
                link = ms.strip()
                try:
                    ...
                    return await a.edit(f"حدث خطأ قم بتوجيه الرسالة الى مطوري @‏z1zzzzzzz1\n{er}")
