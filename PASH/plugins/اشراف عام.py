...
from PASH import CMD_HELP, Alosma
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @xxx_Venus", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@Alosma.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(PASH):
    dc = razan = PASH
    i = 0
    sender = await dc.get_sender()
    me = await PASH.client.get_me()
    ...
    if PASH.is_private:
        user = PASH.chat
        rank = PASH.pattern_match.group(1)
    else:
        PASH.chat.title
    try:
        user, rank = await get_full_user(PASH)
    except:
        pass
    if me == user:
       Alosma = await razan.edit("▾∮ لا استطيع رفع نفسي ،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await PASH.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await PASH.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@Alosma.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(PASH):
    dc = razan = PASH
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       Alosma = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
