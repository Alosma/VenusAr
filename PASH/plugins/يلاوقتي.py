...
from AlAlosma import get_string
...
from PASH import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, Alosma, logging , edit_or_reply
...
autopic_path = os.path.join(os.getcwd(), "PASH", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "PASH", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "PASH", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "PASH", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "PASH", "photo_pfp.png")
...
jep = Config.DEFAULT_PIC or "PASH/helpers/styles/PaybAck.ttf"
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await Alosma.upload_file(autophoto_path)
        try:
            if i > 0:
                await Alosma(
                    functions.photos.DeletePhotosRequest(
                        await Alosma.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await Alosma(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await Alosma.upload_file(auto_group_photo_path)
        try:
            if i > 0:
                async for photo in Alosma.iter_profile_photos(int(dgp), limit=1) :
                    await Alosma(
            ...
            i += 1
            await Alosma(functions.channels.EditPhotoRequest(int(dgp), file))
            os.remove(auto_group_photo_path)
            await asyncio.sleep(60)
        except ChatAdminRequiredError:
            return await Alosma.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
...
async def group_loop():
    ...
    while AUTONAMESTAR:
        ...
        try:
            await Alosma(functions.channels.EditTitleRequest(
                channel=await Alosma.get_entity(int(ag)),
                title=name
            ))
        except ChatAdminRequiredError:
            await Alosma.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
        except ChannelInvalidError:
            return
        except FloodWaitError:
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await Alosma(functions.account.UpdateProfileRequest(last_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await Alosma(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@Alosma.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@Alosma.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@Alosma.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@Alosma.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@Alosma.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
Alosma.loop.create_task(digitalpicloop())
Alosma.loop.create_task(digitalgrouppicloop())
Alosma.loop.create_task(autoname_loop())
Alosma.loop.create_task(autobio_loop())
Alosma.loop.create_task(group_loop())
Alosma.loop.create_task(automessage_loop())