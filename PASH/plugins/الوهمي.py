...
@Alosma.on(admin_cmd(pattern="كتابة(?: |$)(.*)"))
async def _(event):
...
@Alosma.on(admin_cmd(pattern="صوتية(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@Alosma.on(admin_cmd(pattern="فيد(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@Alosma.on(admin_cmd(pattern="لعبة(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@Alosma.on(admin_cmd(pattern="الرابط$"))
async def _(e):
...
@Alosma.on(admin_cmd(pattern="للكل تاك$"))
async def listall(PASH):
    if PASH.fwd_from:
        return
...
@Alosma.on(admin_cmd(pattern=r"سبونج"))
async def kerz(kerz):
    await kerz.edit(R)
...
@Alosma.on(admin_cmd(pattern=r"كلب"))
async def dog(dog):
    await dog.edit(M)
Z = (
    "┈┏━╮╭━┓┈╭━━━━╮\n"
...
@Alosma.on(admin_cmd(pattern=r"ذئب"))
async def fox(fox):
    await fox.edit(H)


@Alosma.on(admin_cmd(pattern=r"فيل"))
async def elephant(elephant):
    await elephant.edit(A)


@Alosma.on(admin_cmd(pattern=r"هومر"))
async def homer(homer):
    await homer.edit(N)


@Alosma.on(admin_cmd(pattern=r"بك"))
async def pig(pig):
    await pig.edit(Z)
