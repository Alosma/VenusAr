...
from Alosma.razan._islam import *
...
@Alosma.on(events.NewMessage(pattern=r"^.اختصر (.+) (.+)"))
...
@Alosma.on(events.NewMessage(pattern=r"^\.(.+)"))
...
@Alosma.on(events.NewMessage(pattern=r"^.قائمه الاختصار|قائمة الاختصار"))
...
@Alosma.on(events.NewMessage(pattern=r"^\.امسح (.+)$"))
