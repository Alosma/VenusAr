...
from PASH import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"PASH/plugins/{shortname}.py")
        checkplugins(path)
        name = "PASH.plugins.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["PASH.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف ✓" + shortname)
...
def remove_plugin(shortname):
    ...
    try:
        name = f"PASH.plugins.{shortname}"
        for i in reversed(range(len(Alosma._event_builders))):
            ev, cb = Alosma._event_builders[i]
            if cb.__module__ == name:
                del Alosma._event_builders[i]
    except BaseException:
        raise ValueError
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"PASH/plugins/assistant/{shortname}.py")
        name = "PASH.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["PASH.plugins.assistant" + shortname] = mod
