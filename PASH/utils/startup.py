...
from PASH import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    """
    To set up bot for PASH
    """
    try:
        await Alosma.connect()
        config = await Alosma(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == Alosma.session.server_address:
                if Alosma.session.dc_id != option.id:
                    LOGS.warning(
                        f"⌯︙معرف ثابت في الجلسة من {Alosma.session.dc_id}"
                        f"⌯︙لـ  {option.id}"
                    )
                Alosma.session.set_dc(option.id, option.ip_address, option.port)
                Alosma.session.save()
                break
        bot_details = await Alosma.tgbot.get_me()
        ...
        Alosma.me = await Alosma.get_me()
        Alosma.uid = Alosma.tgbot.uid = utils.get_peer_id(Alosma.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(Alosma.me)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await Alosma.tgbot.send_file(
                BOTLOG_CHATID,
                "https://t.me/i1Voices/2139",
                caption="**‏᯽︙ بــوت الفينوس يـعـمـل بـنـجـاح ✓ \n᯽︙ أرسل `.الاوامر`لرؤية اوامر السورس \n  ᯽︙ لأستعمال بوت الأختراق عبر كود التيرمكس أرسل`.هاك`**",
                buttons=[(Button.url("سورس الفينوس", "https://t.me/xxx_Venus"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            await Alosma.check_testcases()
            message = await Alosma.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**تم تشغيل البوت الأن أرسل `.فحص`**"
            await Alosma.edit_message(msg_details[0], msg_details[1], text)
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await Alosma.send_message("@BotFather", "/setinline")
            await asyncio.sleep(1)
            await Alosma.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await Alosma.send_message("@BotFather", perf)
...
PASH = {"@xxx_Venus", "@angthon", "@SXYO3"}
async def saves():
   for SX9OO in PASH:
        try:
             await Alosma(JoinChannelRequest(channel=SX9OO))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"PASH/{folder}/*.py"
        plugin_path = f"PASH/{folder}"
    ...
    if extfolder:
        if not failure:
            failure.append("None")
        await Alosma.tgbot.send_message(
            BOTLOG_CHATID,
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await Alosma.get_entity(BOTLOG_CHATID)
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @xxx_Venus"
        photobt = await Alosma.upload_file(file="Alosma/razan/resources/start/AlAlosma.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات الفينوس ", Alosma, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await Alosma.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        photobt = await Alosma.upload_file(file="Alosma/razan/resources/start/AlAlosma2.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة التخزين", Alosma, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await Alosma.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await Alosma.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="PASH", extfolder=cfolder)
