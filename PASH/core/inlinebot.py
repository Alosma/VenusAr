...
def main_menu():
    text = f"**- [سـورس الفينوس](https://t.me/xxx_Venus)\
        \n المسـاعد\
        \n\
        \n◽ الفينوس لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("Alosma"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" PASH UsᴇʀBoᴛ", "https://t.me/xxx_Venus"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./PASH", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس الفينوس 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./PASH", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس الفينوس 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./PASH", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس الفينوس 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© PASH Help",
                description="Help menu for PASH",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/xxx_Venus"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/xxx_Venus",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت الفينوس 🧸♥](https://t.me/xxx_Venus)**\
            \n\
            \n❤ الفينوس هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/xxx_venus)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Alosma.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
