from ANUM import bot, Anime
#By Source saher @an_meie
...
'''
mm = '''
قم بلأنضمام الى قناة الانمي @an_meie
'''
...
keyboard = [
    ...
    [
        Button.url("سورس الانمي🦋", "https://t.me/an_meie")
    ]
]
...
async def repo(event):
    ...
    response = await bot.inline_query(s_6_G, "هاك")
...
async def start(event):
  ...
  if event.sender_id == bot.uid:
    ...
    [
        ...
        Button.url("المطور", "https://t.me/an_meie")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس الانمي", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك الانمي.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
      	await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك الانمي.", buttons=keyboard)
      else:
          await event.reply(f"حدث خطأ قم بتوجيه الرسالة للمطور @s_6_G\n{i}")
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        if changing:
          await event.respond(" @s_6_G لقد تم تغير الرقم بنجاح ✅")
        else:
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/an_meie")
        ]
    ]
