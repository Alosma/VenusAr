...
def main_menu():
    text = f"**- [سـورس الانمي](https://t.me/an_meie)\
        \n المسـاعد\
        \n\
        \n◽ الانمي لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("Anime"):
            buttons = [
                (
                    ...
                    Button.url(" ANUM UsᴇʀBoᴛ", "https://t.me/an_meie"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./ANUM", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس الانمي 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./ANUM", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس الانمي 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./ANUM", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس الانمي 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© ANUM Help",
                description="Help menu for ANUM",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/an_meie"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/an_meie",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت الانمي 🧸♥](https://t.me/an_meie)**\
            \n\
            \n❤ الانمي هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
