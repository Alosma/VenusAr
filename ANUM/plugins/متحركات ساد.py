...
#جميع الحقوق محفوظة لسـورس الانمي تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    s_6_G = await reply_id(event)
    if sad:
        Anime = f"**˛ 𝘼𝙇𝙨𝙖𝙝𝙚𝙧 . .**\n"
        Anime += f"✛━━━━━━━━━━━━━✛\n"
        Anime += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=Anime, reply_to=s_6_G)
