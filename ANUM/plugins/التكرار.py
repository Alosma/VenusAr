...
from ANUM import *
from ANUM import Anime
from ANUM.utils import admin_cmd
...
async def spam_function(event, ANUM, Anime, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(Anime[0])
    if len(Anime) == 2:
        spam_message = str(Anime[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await ANUM.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and ANUM.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            ANUM = await event.client.send_file(
                event.chat_id, ANUM, caption=ANUM.text
            )
            await _catutils.unsavegif(event, ANUM)
        ...
        if BOTLOG:
            ...
            ANUM = await event.client.send_file(BOTLOG_CHATID, ANUM)
            await _catutils.unsavegif(event, ANUM)
        return
    elif event.reply_to_msg_id and ANUM.text:
        spam_message = ANUM.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@Anime.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    ANUM = await event.get_reply_message()
    Anime = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(Anime[0])
    except Exception:
    ...
    await spam_function(event, ANUM, Anime, sleeptimem, sleeptimet)

@Anime.on(admin_cmd(pattern=f"{Mukrr}"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, Anime, sleeptimem, sleeptimet, DelaySpam=True)


@Anime.ar_cmd(pattern="تكرار الملصق$")
...
@Anime.on(events.NewMessage(pattern=r"^.تكرار (\d+) (\d+) (.+)"))
async def share_messages(event):
    ...
    try:
        entity = await Anime.get_entity(group_link)
    except ValueError:
    ...
    for _ in range(message_count):
        sent_message = await Anime.send_message(entity, reply_message.text)
        await asyncio.sleep(time_interval)

    await event.edit(f"⌔∮  تم التكرار بنجاح")

@Anime.ar_cmd(pattern="وسبام (.*)")
async def tmeme(event):
...
@Anime.on(events.NewMessage(pattern=r"^.نقل (\d+) (\d+) (.+)"))
async def share_messages(event):
    time_interval = int(event.pattern_match.group(1))
    message_count = int(event.pattern_match.group(2))
    group_link = event.pattern_match.group(3)
    ...
    for _ in range(message_count):
        await Anime.forward_messages(entity, reply_message)
        await asyncio.sleep(time_interval)

    await event.reply(f"⌔∮ تم التكرار بنجاح")

@Anime.ar_cmd(pattern="ايقاف التكرار ?(.*)")
