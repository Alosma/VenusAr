...
from ANUM import CMD_HELP, Anime
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @an_meie", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@Anime.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(ANUM):
    dc = razan = ANUM
    i = 0
    sender = await dc.get_sender()
    me = await ANUM.client.get_me()
    ...
    if ANUM.is_private:
        user = ANUM.chat
        rank = ANUM.pattern_match.group(1)
    else:
        ANUM.chat.title
    try:
        user, rank = await get_full_user(ANUM)
    except:
        pass
    if me == user:
       Anime = await razan.edit("▾∮ لا استطيع رفع نفسي ،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await ANUM.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await ANUM.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@Anime.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(ANUM):
    dc = razan = ANUM
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       Anime = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
