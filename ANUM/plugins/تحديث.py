...
from ANUM import HEROKU_APP, UPSTREAM_REPO_URL, Anime
...
IFFUCI_ACTIVE_BRANCH_NAME = "ANUM"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/ANUM"
...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**᯽︙ قام مطورين السورس بتحديث الانمي**\n᯽︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس الانمي بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [s_6_G.chat_id, s_6_G.id])
    except Exception as e:
        LOGS.error(e)
    ups_rem.fetch(ac_br)
    repo.git.reset("--hard", "FETCH_HEAD")
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/ANUM", force=True)
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس الانمي انـتـظـر**")
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("ANUM", origin.refs.ANUM)
        repo.heads.ANUM.set_tracking_branch(origin.refs.ANUM)
        repo.heads.ANUM.checkout(True)
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس الانمي ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس الانمي انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ جارِ تحديث ريبو التنصيب لسورس الانمي **")
    ...
    try:
        ...
        origin = repo.create_remote("upstream", off_repo)
        origin.fetch()
        repo.create_head("ANUM", origin.refs.master)
        repo.heads.ANUM.set_tracking_branch(origin.refs.master)
        repo.heads.ANUM.checkout(True)
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("**᯽︙ جارِ اعادة تنصيب سورس الانمي, انتظر قليلاً ..**")
    await deploy(event, repo, ups_rem, ac_br, txt)


progs = [6330221767,6299015318,5762222122,5835316914,6528926431]
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("** ᯽︙ يتم تحديث سورس الانمي بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
