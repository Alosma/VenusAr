...
@Anime.on(admin_cmd(pattern="(خط الغامق|خط غامق)"))
async def btext(event):
    ...
    if isbold:
        ...
        return
@Anime.on(admin_cmd(pattern="(خط البرمجة|خط برمجة)"))
async def btext(event):
    ...
    if isprogramming:
        ...
        return
@Anime.on(admin_cmd(pattern="(خط رمز|خط الرمز)"))
async def btext(event):
...
@Anime.on(admin_cmd(pattern="(خط بايثون|خط بايثون)"))
async def btext(event):
    ...
    if ispython:
        ...
        return
@Anime.on(events.NewMessage(outgoing=True))
