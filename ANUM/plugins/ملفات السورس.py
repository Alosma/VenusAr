...
@Anime.ar_cmd(
    ...
    info={
        "header": "To list all plugins in ANUM.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in ANUM"
    cmd = "ls ANUM/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[الانمي](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@Anime.ar_cmd(
    ...
    info={
        "header": "To list all environment values in ANUM.",
        "description": "to show all heroku vars/Config values in your ANUM",
        "usage": "{tr}env",
    },
) 
async def _(event):
    "To show all config values in ANUM"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
    ...
    await Anime.send_message("me", OUTPUT)
...
@Anime.ar_cmd(pattern="رابط مباشر")
