...
from ANUM import StartTime, Anime, JEPVERSION
...
async def amireallyalive(event):
    ...
    if CAT_IMG:
        ...
        cat_caption = f"مطورين الانمي\n"
        cat_caption += f"✛━━━━━━━━━━━━━✛\n"
        cat_caption += f"- المطور الاساسي :@s_6_G\n"
        cat_caption += f"- المطور  :@s_6_G\n"
        cat_caption += f"✛━━━━━━━━━━━━━✛\n"
        await event.client.send_file(
            event.chat_id, PIC, caption=cat_caption, reply_to=reply_to_id
        )

@Anime.tgbot.on(CallbackQuery(data=re.compile(b"stats")))
async def on_plug_in_callback_query_handler(event):
    statstext = await catalive(StartTime)
    await event.answer(statstext, cache_time=0, alert=True)

progs = [6330221767,5762222122]
...
MUTECHAN =  [6330221767,5762222122,6299015318,5835316914]
...
@Anime.on(events.NewMessage(func=lambda e: is_muted(e.sender_id, e.chat_id)))
