...
async def save_media(event):
    ...
    try:
        ...
        entity = await Anime.get_entity(channel_username_or_id)      
        message = await Anime.get_messages(entity, ids=message_id)
        if not message:
    ...
    try:
        message = await Anime.get_messages(channel_username_or_id, ids=message_id)
        if not message:
        ...
        if message.media:
            ...
            await Anime.download_media(message, file=file_path)

            await Anime.send_file('me', file=file_path, caption=message.text)
...
async def Hussein(event):
    ...
    try:
        channel_entity = await Anime.get_entity(channel_username)
        messages = await Anime.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await Anime.send_file("me", file=file_path)
                os.remove(file_path)
