...
@Anime.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        ANUM = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        ANUM = str(jok).replace("-100", "")
    else:
        try:
            ANUM = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await Anime.send_message(int(chat), event.message)
