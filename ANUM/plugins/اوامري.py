...
from Anime.razan.resources.assistant import *
from Anime.razan.resources.mybot import *
from ANUM import Anime
from ..core import check_owner
from ..Config import Config

JEP_IC = "https://telegra.ph/file/56a3dd726306259beded6.jpg"
ROE = "**♰ هـذه هي قائمة اوامـر سـورس الانمي ♰**"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == bot.uid:
            buttons = [
                [Button.inline("🦋 اوامر الادمن 🦋", data="Anime0")],
                [
...
async def repo(event):
    if event.fwd_from:
        return
    s_6_G = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await bot.inline_query(s_6_G, "اوامري")
    await response[0].click(event.chat_id)
    await event.delete()


@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"Anime0")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"jrzst")))
@check_owner
async def _(event):
    butze = [
    [
     Button.inline("التالي", data="tslrzj"),
     Button.inline("رجوع", data="Anime0")]]
    await event.edit(GRTSTI, buttons=butze)


@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"tslrzj")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"krrznd")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"rozbot")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"Jmrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"r7brz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"sejrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"gro")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"grrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"iiers")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"rfhrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@Anime.tgbot.on(CallbackQuery(data=re.compile(rb"uscuxrz")))
@check_owner
async def _(event):
    buttons = [[Button.inline("رجوع", data="Anime0"),]]
