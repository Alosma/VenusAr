...
async def Username_exists_by_Anime(username):
    try:
        entity = await Anime.get_entity(username)
...
@Anime.on(events.NewMessage(pattern=r"^\.ثلاثي (\d+)$"))
async def generate_random_usernames(event):
    ...
    while count > 0:
        ...
        if not await Username_exists_by_Anime(username):
...
@Anime.on(events.NewMessage(pattern=r"^\.رباعي (\d+)$"))
async def generate_random_usernames(event):
    ...
    if generated_usernames:
        usernames_text = "\n".join([f"@{username}" for username in generated_usernames])
        await event.edit(f"**᯽︙ تم انشاء {len(generated_usernames)} يوزر جديد**\n\n{usernames_text}")
@Anime.on(events.NewMessage(pattern=r"^\.يوزربوت (\d+)$"))
async def generate_random_usernames(event):
...
@Anime.on(events.NewMessage(pattern=r"^\.خماسي (\d+)$"))
async def generate_random_usernames(event):
    ...
    if generated_usernames:
        usernames_text = "\n".join([f"@{username}" for username in generated_usernames])
        await event.edit(f"**᯽︙ تم انشاء {len(generated_usernames)} يوزر جديد**\n\n{usernames_text}")
@Anime.on(events.NewMessage(pattern=r"^\.سداسي (\d+)$"))
async def generate_random_usernames(event):
...
async def handle_messages(event):
    ...
    if user_id in allowed_users:
        if message_text == 'منصبين؟':
            ...
            await Anime(SendMessageRequest('@Y7os6', 'كل عام وانت بخير \n Anime'))